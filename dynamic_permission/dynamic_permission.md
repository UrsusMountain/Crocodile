...
将我们上面写的打电话的功能用 AndPermission 重写一遍  
...
#### 局限性（作者可能有自己的考虑）
1. 没有将“不再询问”封装成一个回调。
需要在 `onFailed` 里面手动处理，该库提供了一个[`SettingDialog`](https://github.com/yanzhenjie/AndPermission/blob/master/README-CN.md#提示用户在系统设置中授权)的功能来处理不再询问，不过和自己手动打开应用管理界面区别不大。没法做到在权限申请前处理“不再询问”

2. `PermissionActivity`中只有一个静态的`PermissionListener`对象，注册一个就等同于取消另一个，所以无法同时进行多个权限申请  
3. 由于是在一个新的 `PermissionActivity` 中做的申请权限申请结果处理，虽然

  
## 参考文章
[android官方动态权限文档](https://developer.android.com/training/permissions/requesting.html)  

[addPermission项目文档](https://github.com/yanzhenjie/AndPermission/blob/master/README-CN.md)
